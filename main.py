...
def start_extraction(request: ExtractRequest):
    ...
    # Using Python < 3.9 compatible dict merge
...
def process_acapella(link, job_id):
    ...
    try:
        ...
        ydl_opts = {
            ...
            # Keep potentially helpful options (effectiveness varies)
        ...
        with YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                if job_store.get(job_id, {}).get("status") == "error":
                    ...
                    return
                print(f"Job {job_id}: Attempting yt-dlp extract_info...") # Added log
            ...
            # Catch specific yt-dlp DownloadError
                ...
                if "confirm you.re not a bot" in str(ydl_error).lower() or \
                   "unable to download webpage" in str(ydl_error).lower() or \
                   "sign in" in str(ydl_error).lower():
                    error_message = "Download failed: Likely blocked by YouTube verification."
                else:
                    error_message = f"Download failed: {str(ydl_error)[:200]}"
                raise ValueError(error_message) # Raise specific error message
        ...
        if not output_wav.exists():
             print(f"Job {job_id}: Error - Expected WAV file {output_wav} not found after yt-dlp finished (check logs for download errors).")
             raise FileNotFoundError(f"Output WAV {output_wav.name} missing after download completed.")
    ...
    if d['status'] == 'downloading':
        if job_store[job_id]['status'] == 'downloading':
            ...
            if total_bytes and total_bytes > 0:
                ...
                job_store[job_id]['progress'] = min(5 + download_progress, 49)
